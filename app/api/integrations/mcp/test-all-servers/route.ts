...
/**
 * Comprehensive test of all MCP servers
 * Tests each server with proper MCP protocol (initialize + tools/list)
 */
  ...
  try {
    ...
    if (!initResponse.ok) {
      ...
      return {
        ...
        success: false,
        error: `Initialize failed: ${initResponse.status} ${initResponse.statusText}${errorText ? ` - ${errorText.substring(0, 200)}` : ""}`,
    ...
    const initData = parseSSEResponse(initResponseText)
    ...
    // Extract session ID from initialize response header (MCP spec requirement)
    const sessionId = 
    ...
    // Include session ID in tools/list request if provided
    ...
    if (!toolsResponse.ok) {
      ...
      return {
        ...
        serverName: server.name,
        success: false,
        error: `Tools/list failed: ${toolsResponse.status} ${toolsResponse.statusText}${errorText ? ` - ${errorText.substring(0, 200)}` : ""}`,
        responseTime: Date.now() - startTime,
      }
    }
    ...
    const toolsData = parseSSEResponse(toolsResponseText)
...
export async function GET() {
  try {
    console.log("[MCP Test All] Testing all MCP servers...")
    ...
    console.log(`[MCP Test All] Results: ${successful.length}/${results.length} successful`)
    ...
    console.error("[MCP Test All] Error:", error)
    return NextResponse.json(
      {
        success: false,

-- Seed Data Migration
-- This creates seed data for all Notion sync tables
-- Can be triggered via API endpoint /api/admin/seed-data

-- Note: This migration creates a function that can be called with a user_id
-- The actual seeding is done via API endpoint for security

CREATE OR REPLACE FUNCTION seed_forge_data(target_user_id UUID)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
BEGIN
  ...
  RETURNING id INTO STRICT char_count;
...
GRANT EXECUTE ON FUNCTION seed_forge_data(UUID) TO authenticated;

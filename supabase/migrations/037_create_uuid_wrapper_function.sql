-- Create a wrapper function that accepts UUID and casts internally
-- This ensures compatibility with Supabase RPC which may pass UUID type

CREATE OR REPLACE FUNCTION seed_forge_data_uuid(target_user_id UUID)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
BEGIN
  -- Cast UUID to TEXT and call the TEXT version
  RETURN seed_forge_data(target_user_id::TEXT);
END;
$$;

-- Also ensure the TEXT version exists and is correct
...
BEGIN
  ...
  SELECT
    user_id_text,
    NULL::TEXT,  -- notion_page_id: NULL until synced to Notion
    gen_random_uuid()::TEXT,  -- notion_database_id: temporary UUID, will be updated during sync
    name,
    description,
    properties,
    NOW(),

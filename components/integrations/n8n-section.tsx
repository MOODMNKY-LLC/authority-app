...
import { Zap, Key, Loader2, Check, RefreshCw, ExternalLink, CheckCircle2, XCircle, Info, HelpCircle } from "lucide-react"
...
// Hardcoded N8n host (from .env)
const N8N_HOST = process.env.NEXT_PUBLIC_N8N_HOST || "https://slade-n8n.moodmnky.com"
...
export function N8nSection() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (hasKey && !data.verified) {
          console.log("[Automations] Auto-verifying API key...")
          await handleVerify()
        }
      }
    } catch (error) {
      console.error("Failed to load Automations settings:", error)
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleVerify = async (showToast = true) => {
    ...
    if (!apiKey && !isMaskedKey) {
      if (showToast) {
        toast({
          ...
          description: "Please enter your Automations API key first.",
          variant: "destructive",
        })
      }
      return
    ...
    if (isMaskedKey && !showToast) {
      try {
        const response = await fetch("/api/integrations/n8n/verify", {
          method: "POST",
        ...
        console.error("[Automations] Auto-verification failed:", error)
        setVerified(false)
      }
      return
    }
      ...
      if (data.verified) {
        ...
        if (showToast) {
          toast({
            ...
            description: `Connected as ${data.userInfo?.name || data.userInfo?.email || "Automations user"}`,
          })
        }
  ...
  const handleSave = async () => {
    if (!apiKey || apiKey === "••••••••") {
      toast({
        title: "API Key Required",
        description: "Please enter your Automations API key.",
        variant: "destructive",
      })
      return
    }
    ...
    try {
      // Save the API key
      const saveResponse = await fetch("/api/integrations/n8n", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      const verifyResponse = await fetch("/api/integrations/n8n/verify", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      if (verifyData.verified) {
        ...
        toast({
          title: "Automations settings saved & verified",
          description: `Connected as ${verifyData.userInfo?.name || verifyData.userInfo?.email || "Automations user"}`,
        })
        // Auto-fetch workflows after successful verification
      ...
      toast({
        title: "Error",
        description: error.message || "Failed to save Automations settings",
        variant: "destructive",
      })
    } finally {
      setSaving(false)
  ...
  const fetchWorkflows = async () => {
    ...
    if (!verified) {
      ...
      if (!verified) {
        toast({
          ...
          description: "Please verify your API key first before fetching workflows.",
          variant: "destructive",
        })
        return
      }
    ...
    setFetching(true)
    try {
      const response = await fetch("/api/integrations/n8n/workflows")
      if (response.ok) {
        const data = await response.json()
        setWorkflows(data.workflows || [])
        toast({
          title: "Workflows loaded",
          description: `Found ${data.workflows?.length || 0} workflows.`,
        })
      } else {
      ...
      toast({
        title: "Error",
        description: error.message || "Failed to fetch workflows. Your API key may need to be re-verified.",
        variant: "destructive",
      })
    } finally {
      setFetching(false)
  ...
  return (
    <div className="space-y-6">
      <div className="p-4 rounded-lg bg-gradient-to-r from-orange-900/20 to-zinc-900/20 border border-orange-900/30">
        <h2 className="text-2xl font-semibold text-white mb-2 flex items-center gap-2">
          <Zap className="h-6 w-6 text-orange-400" />
          Automations
        </h2>
        <p className="text-zinc-400 text-sm">
          Connect to your automation platform to manage and execute workflows. Your API key is encrypted and stored securely.
        </p>
      </div>
          ...
          <CardTitle className="text-white flex items-center gap-2">
            ...
            <Tooltip>
              ...
              <TooltipContent className="max-w-xs">
                <p className="font-semibold mb-1">Automations API Key</p>
                <p className="text-xs">
                  Your automation platform API key allows Authority to access your workflows. Find it in your automation platform settings (Settings → API).
                  The key is encrypted before storage and can only be decrypted by the application.
                </p>
              </TooltipContent>
            </Tooltip>
          </CardTitle>
          <CardDescription>Configure your N8n API connection</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="space-y-2">
            <Label htmlFor="n8n_api_key" className="flex items-center gap-2">
              API Key
              <Tooltip>
                <TooltipTrigger asChild>
                ...
                <TooltipContent>
                  <p className="text-xs">Enter your Automations API key. It will be encrypted and stored securely.</p>
                </TooltipContent>
              </Tooltip>
            </Label>
            <div className="flex gap-2">
              <Input
                ...
                placeholder={verified ? "API key saved (click to change)" : "Enter your Automations API key"}
          ...
          <div className="space-y-2">
            <Label className="flex items-center gap-2">
              ...
              <Tooltip>
                ...
                <TooltipContent>
                  <p className="text-xs">This is the Authority automation platform instance URL (configured by administrators).</p>
                </TooltipContent>
              </Tooltip>
            </Label>
            <div className="p-3 rounded-md bg-zinc-950/50 border border-zinc-800/50">
              <div className="flex items-center justify-between">
                <span className="text-sm text-zinc-300">{N8N_HOST}</span>
              ...
              <div className="flex items-start gap-2">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-green-300/80">
                    {userInfo ? (
                      <>
                        Connected as: <span className="font-medium">{userInfo.name || userInfo.email || "Automations user"}</span>
                      </>
                    ) : (
        ...
        <CardContent>
          {!verified ? (
            <div className="text-center py-8 text-zinc-500 text-sm">
              Verify your API key to load workflows
            </div>
          ) : workflows.length > 0 ? (
            <ScrollArea className="h-[300px]">
              <div className="space-y-2">
                {workflows.map((workflow: any) => (
                  <div
                    key={workflow.id}
                    className="p-3 rounded-md bg-zinc-950/50 border border-zinc-800/50 hover:border-orange-800/50 transition-colors"
                  >
                    <div className="flex items-start justify-between">
                        ...
                        {workflow.tags && workflow.tags.length > 0 && (
                          <div className="flex gap-2 mt-2">
                            {workflow.tags.map((tag: any) => (
                              <Badge key={tag.id || tag} variant="outline" className="text-xs">
                                {tag.name || tag}
                              </Badge>
                            ))}
                          </div>
                      ...
                      <div className="flex gap-2">
                        <a
                          href={`${N8N_HOST}/workflow/${workflow.id}`}
                          target="_blank"
                          rel="noopener noreferrer"
                          className="text-orange-400 hover:text-orange-300"
                          title="Open in Automation Platform"
                        >
            ...
            <div className="text-center py-8 text-zinc-500 text-sm">
              Click Refresh to load your workflows
            </div>
          )}
        </CardContent>
      </Card>
